...
export function Header({ className }: HeaderProps) {
  ...
  return (
    <header
      className={cn("sticky top-2 z-10 my-4 flex items-center px-2", "md:top-4 md:my-8", className)}
    >
      <nav className="flex h-12 w-full items-center justify-between rounded-full border border-white/40 bg-white/80 p-2 text-sm font-medium text-gray-500 shadow-lg shadow-black/[0.03] backdrop-blur-sm">
        <div className="flex h-full items-center justify-center overflow-hidden rounded-full">
          {isHomePage ? (
            ...
            <Link href="/" className="relative aspect-square size-full overflow-hidden bg-blue-200">
              <Image
                src={AVATAR_IMAGE}
                alt="Lê Trung Lực"
                fill
        ...
        <ul className="flex h-full items-center gap-2 font-mono text-sm tracking-tight">
          {NAV_ITEMS.map((item) => {
            const isActiveRoute =
              item.title === "home" ? pathname === item.url : pathname.startsWith(item.url);
              ...
              <li key={item.url} className="relative flex h-full items-center px-4">
                {isActiveRoute && (
                  <motion.div
                    ...
                    className="absolute inset-0 size-full rounded-full bg-gray-100"
                  />
                )}

                <Link
                  href={item.url}
                  className={cn("z-10", "hover:text-gray-950", { "text-gray-950": isActiveRoute })}
                >
                  {item.title}
                </Link>
              </li>
